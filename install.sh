...
#
# Install RDK Device Skills into the user-home and project dirs that
# Claude Code, Codex, Cursor, and Qoder read from.
#
# Safe to re-run: correct links are left untouched, and stale RDK links are
...
#   ./install.sh --force                              # replace existing RDK skill dirs
#
# Targets:
...
install_entry() {
  ...
  if [ "$MODE" = symlink ]; then
    if [ -L "$dst" ]; then
      ...
      if [ "$FORCE" -eq 1 ] && is_rdk_entry "$dst"; then
        skip "$name (replacing existing RDK entry because --force was passed)"
        rm -rf "$dst"
      else
        err "$name — $dst exists and is not a symlink; skipping (use --force to replace RDK entries)"
        return
      fi
    fi
    ln -s "$src" "$dst"
    ...
    if [ -e "$dst" ]; then
      if [ "$FORCE" -eq 1 ] && is_rdk_entry "$dst"; then
        ...
        err "$name — $dst exists; skipping (use --force to replace RDK entries)"
        return
      fi
    fi
    if [ -d "$src" ]; then
...
echo "If an agent still does not expose RDK skills, confirm it reads the target root shown above."
